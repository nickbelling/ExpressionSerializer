 ...
 */
export function serializeExpression<TEntity>(
    expression: (x: TEntity) => boolean, 
    throwOnEnter: boolean = false): string {
    const error: string = `This function is intended to be replaced during compile-time, and should not exist at runtime.
    If you encounter this error during runtime, it means you have not correctly set up the TypeScript transformer
    included with 'ts-lambda-to-odata'. See that project's README for details of how to implement it.`;
