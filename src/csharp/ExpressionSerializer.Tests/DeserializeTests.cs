...
public class DeserializeTests
{
    ...
    {
        Func<Person, bool> filter = ExpressionSerializer.Deserialize<Person>("Age gt 20");
    ...
    {
        Func<Person, bool> filter = ExpressionSerializer.Deserialize<Person>("Age gt 5 and startswith(Name, 'B')");
    ...
    {
        Func<Customer, bool> filter = ExpressionSerializer.Deserialize<Customer>("Person/Name eq 'Bob'");
    ...
    {
        Func<Person, bool> filter = ExpressionSerializer.Deserialize<Person>("Age gt 18 or Name eq 'Alice'");
        Assert.IsTrue(filter(_person1));
        Assert.IsFalse(filter(_person2));
    ...
    {
        Func<Person, bool> greaterThanFilter = ExpressionSerializer.Deserialize<Person>("Age gt 20");
        Assert.IsTrue(greaterThanFilter(_person1)); // Age 25
        Assert.IsFalse(greaterThanFilter(_person2)); // Age 18

        Func<Person, bool> lessThanFilter = ExpressionSerializer.Deserialize<Person>("Age lt 20");
        Assert.IsFalse(lessThanFilter(_person1)); // Age 25
        Assert.IsTrue(lessThanFilter(_person2)); // Age 18

        Func<Person, bool> notEqualFilter = ExpressionSerializer.Deserialize<Person>("Age ne 18");
    ...
    {
        Func<Person, bool> startsWithFilter = ExpressionSerializer.Deserialize<Person>("startswith(Name, 'A')");
        Assert.IsTrue(startsWithFilter(_person1)); // Name "Alice"
        Assert.IsFalse(startsWithFilter(_person2)); // Name "Bob"

        Func<Person, bool> endsWithFilter = ExpressionSerializer.Deserialize<Person>("endswith(Name, 'e')");
        ...
        Func<Person, bool> containsFilter = ExpressionSerializer.Deserialize<Person>("contains(Name, 'lic')");
    ...
    {
        Func<Order, bool> complexFilter = ExpressionSerializer.Deserialize<Order>(
            "Customer/Person/Age gt 25 and Items/any(i: i/HistoricPrices/any(p: p gt 150))");
        Assert.IsTrue(complexFilter(_order1));

        Func<Order, bool> negativeComplexFilter = ExpressionSerializer.Deserialize<Order>(
    ...
    {
        Func<Order, bool> containsFilter = ExpressionSerializer.Deserialize<Order>(
            "Items/any(i: i/HistoricPrices/any(p: p eq 200))");
        Assert.IsTrue(containsFilter(_order1)); // Contains price 200

        Func<Order, bool> notContainsFilter = ExpressionSerializer.Deserialize<Order>(
    ...
    {
        Func<Order, bool> filter = ExpressionSerializer.Deserialize<Order>("Customer/Person/Name eq 'Alice'");
        Assert.IsTrue(filter(_order1));

        filter = ExpressionSerializer.Deserialize<Order>("Customer/Person/Age lt 18");
    ...
    {
        Func<Order, bool> filter = ExpressionSerializer.Deserialize<Order>(
            "Customer/Person/Age gt 25 and Items/any(i: i/HistoricPrices/any(p: p gt 150))");
        Assert.IsTrue(filter(_order1));

        filter = ExpressionSerializer.Deserialize<Order>(
            "Customer/Person/Age gt 25 and Items/any(i: i/HistoricPrices/any(p: p lt 100))");
        Assert.IsFalse(filter(_order1));
    }
}
