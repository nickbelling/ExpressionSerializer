...
public class SerializeTests
{
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(p => p.Age > 20);
        AssertAreEqual("Age gt 20", filter);

        filter = ExpressionSerializer.Serialize<Person>(p => p.Name.StartsWith("B"));
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Customer>(c => c.Person.Name == "Bob");
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(p => p.Age < 30);
        AssertAreEqual("Age lt 30", filter);

        filter = ExpressionSerializer.Serialize<Person>(p => p.Name.EndsWith("Smith") == false);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(p => p.Age > 5 && p.Name.StartsWith("B"));
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(p => p.Age > 18 || p.Name == "Alice");
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Age > 15);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Age < 18);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Age == 30);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Age != 0);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name == "Bob");
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name == null);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name.StartsWith("B"));
        AssertAreEqual("startswith(Name,'B')", filter);
    }
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name.EndsWith("B"));
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name.Contains("ob"));
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name.Substring(1, 2) == "ob");
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name.IndexOf("b") == 2);
        AssertAreEqual("indexof(Name, 'b') eq 2", filter);
        Assert.ThrowsException<NotSupportedException>(() => ExpressionSerializer.Serialize<Person>(x => x.Name.IndexOf('b') == 2));
    }

    [TestMethod]
    public void Test_string_Length()
    {
        string? filter = ExpressionSerializer.Serialize<Person>(x => x.Name.Length == 3);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Customer>(c => c.IsActive == true);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Customer>(c => c.IsActive);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Item>(x => x.HistoricPrices.Count > 5);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Order>(x => x.Items.Any(i => i.HistoricPrices.Count > 1));
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Item>(x => x.HistoricPrices.All(p => p > 10));
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Order>(x => x.Items.Any(i => i.HistoricPrices.Any(p => p == 11)) == false);
    ...
    {
        string? filter = ExpressionSerializer.Serialize<Person>(p => p.Age > GetNumber(10));
    ...
    {
        int number = 15;
        string? filter = ExpressionSerializer.Serialize<Person>(p => p.Age > number);
        AssertAreEqual("Age gt 15", filter);
    }
